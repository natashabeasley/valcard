# Producing QC figures for val card RNA+ATAC 
set.seed(42)
library(Seurat)
library(Signac)
library(future)
...
ggplot(cell_counts, aes(Sample, Cells, fill = Sample)) +
  geom_col(width = 0.9) +
...
ggplot(meta_all, aes(x = orig.ident, y = percent.mt, fill = orig.ident)) +
...
ggplot(meta_all, aes(x = orig.ident, y = nCount_RNA, fill = orig.ident)) +
...
ggplot(meta_all, aes(x = orig.ident, y = nFeature_RNA, fill = orig.ident)) +
...
# Collect ATAC metadata from each sample 
...
# ATAC QC plots (visualize only) 
## Number of ATAC features per cell
violin_jitter(meta_atac, "nFeature_ATAC", "Number of ATAC Features per Cell", "ATAC features")

## Fraction of Reads in Peaks (FRiP) — will be blank if FRiP not available
violin_jitter(meta_atac, "FRiP", "Fraction of Reads in Peaks per Cell", "Fraction in Peaks")

## Nucleosome signal per cell
violin_jitter(meta_atac, "nucleosome_signal", "Nucleosome Signal per Cell", "Nucleosome Signal")

## Fragment count per cell
violin_jitter(meta_atac, "nCount_ATAC", "Fragment Count per Cell", "Fragments / Cell")

## TSS enrichment
violin_jitter(meta_atac, "TSS.enrichment", "TSS Enrichment", "TSS Enrichment")

# Per-sample averages (printed) 
...
# create filtered objects now, if/when you’re ready 
