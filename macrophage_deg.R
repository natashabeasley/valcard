...
save_deg <- function(df, label,
                     outdir = "/rfs/CardiacSurgeryRes/nb443/VALCARD/macrophage/deg_results_balanced") {
...
ggplot(df_bal, aes(contrast, n, fill = direction)) +
  ...
  labs(x = NULL, y = "Number of significant DEGs (FDR < 0.05, |log2FC| ≥ 0.25)",
       title = "Significant DEGs - Macrophages") +
...
VlnPlot(cm_bal, features = c("PAX8-AS1"), group.by = "group",
...
go_bp_view_and_save <- function(deg, label,
                                outdir = "/rfs/CardiacSurgeryRes/nb443/VALCARD/macrophage/go_enrichment",
                                top_show = 20) {
